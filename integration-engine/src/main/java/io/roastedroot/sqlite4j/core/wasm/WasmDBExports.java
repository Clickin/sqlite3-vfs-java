// Modified for sqlite3-vfs: SQLite allocator ownership and callback teardown.
package io.roastedroot.sqlite4j.core.wasm;

import static io.roastedroot.sqlite4j.core.Codes.SQLITE_NULL;
import static io.roastedroot.sqlite4j.core.Codes.SQLITE_NOMEM;
...
public class WasmDBExports {
    ...
    public WasmDBExports(Instance instance) {
        ...
        this.malloc = instance.exports().function("sqlite3_malloc");
        this.free = instance.exports().function("sqlite3_free");
        ...
        this.totalChanges = instance.exports().function("sqlite3_total_changes64");
    ...
    public int malloc(int size) throws SQLException {
        int ptr = (int) malloc.apply(Math.max(1, size))[0];
        // https://www.sqlite.org/c3ref/free.html: NULL is allocation failure, not guest storage.
        if (ptr == 0) {
    ...
    public StringPtrSize allocString(String str) throws SQLException {
    ...
    // with trailing 0
    public int allocCString(String str) throws SQLException {
        byte[] bytes = str.getBytes(StandardCharsets.UTF_8);
        int strBytesPtr = malloc(bytes.length + 1);
        try {
            instance.memory().write(strBytesPtr, bytes);
            instance.memory().writeByte(strBytesPtr + bytes.length, (byte) 0);
            return strBytesPtr;
        } catch (RuntimeException | Error failure) {
            free(strBytesPtr);
            throw failure;
    ...
    public long totalChanges(int dbPtr) {
        return totalChanges.apply(dbPtr)[0];
    }
    ...
    public int createNullFunction(int dbPtr, int namePtr) {
        return (int)
                createFunction.apply(dbPtr, namePtr, -1, SQLITE_UTF16, 0, 0, 0, 0, 0)[0];
    }
    ...
    public void progressHandler(int dbPtr, int vmCalls, int userData) {
        progressHandler.apply(dbPtr, vmCalls, userData == 0 ? 0 : xProgressPtr, userData);
    }

    public void busyHandler(int dbPtr, int userData) {
        busyHandler.apply(dbPtr, userData == 0 ? 0 : xBusyPtr, userData);
    }
    ...
    public int destroyCollation(int dbPtr, int zNamePtr) {
        return (int) createCollation.apply(dbPtr, zNamePtr, SQLITE_UTF8, 0, 0, 0)[0];
    }
