...
public abstract class Function {
    ...
    /**
     * Registers a given function with the connection.
     * A Function instance belongs to one connection; use a separate instance for another connection.
     *
     * @param conn The connection.
     * @param name The name of the function.
     * @param f The function to register.
    ...
    public static void create(Connection conn, String name, Function f, int nArgs, int flags)
        ...
        // Never hold the Function monitor while entering a DB: callbacks acquire them in reverse.
        synchronized (f) {
            if (f.db != null && f.db != database) {
                throw new SQLException("Function instance is already bound to another connection");
            }
            f.conn = sqliteConnection;
            f.db = database;
        }

        int rc = database.create_function(name, f, nArgs, flags);
        if (rc != Codes.SQLITE_OK) {
            throw DB.newSQLException(rc, "error creating function");
        }
    }
